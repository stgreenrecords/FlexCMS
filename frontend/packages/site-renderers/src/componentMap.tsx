/**
 * The single renderer registry for TUT components.
 *
 * This used to live inside the public site app, which meant the admin editor could
 * not reach it and grew a parallel set of hand-written previews instead. The two
 * drifted, as duplicated renderers always do: components with real UI on the site —
 * product grids, feature lists, featured content — appeared in the editor as grey
 * boxes with the component's name in them. Authors were editing a wireframe.
 *
 * Living in a package, one registry serves both, so the editor canvas is WYSIWYG by
 * construction rather than by anyone remembering to update two places.
 *
 * The contracts are injected rather than imported, because the generated contract
 * file is a design artifact whose path differs per app; the package stays free of
 * assumptions about where it sits on disk.
 */
'use client';
...
/** Props every renderer in this registry receives. */
export interface TutRendererProps {
...
export type TutRenderer = React.ComponentType<TutRendererProps>;
...
const ContractFallback = ({ data, resourceType, name }: TutRendererProps) => (
  <section
...
/** Renderers registered against an exact resourceType. */
export const explicitTutRenderers: Record<string, TutRenderer> = {
...
/**
 * Builds the registry for a set of component contracts.
 *
 * Every contract gets an entry — a group renderer when nothing more specific exists —
 * and the explicit renderers above are layered on top, so a component with real UI
 * always wins over its group's generic rendering.
 */
export function createTutComponentMap(
  contracts: TutComponentContract[],
): ComponentMapper<TutRenderer> {
  const DefaultTutRenderer = defaultTutRenderer;

  const TutAwareFallback = (props: TutRendererProps) => {
  ...
  return new ComponentMapper<TutRenderer>()
    .registerAll(buildTutRendererEntries(contracts))
    .registerAll(explicitTutRenderers)
